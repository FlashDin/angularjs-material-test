...
function loginCtrl($scope, LoginService, $interval, $rootScope, $location) {
    ...
    LoginService.dataQr()
        ...
        .catch(function (response) {
            console.error('Error dataQr:', response.status, response.data);
        })
        .finally(function () {
    ...
    // stops the interval
    $scope.stopRealtimeQr = function () {
        $interval.cancel(realtimeQr);
    };
    $scope.stopRealtimeUserSession = function () {
        $interval.cancel(realtimeUserSession);
    };
    var rand = 0;
    $scope.dataQr = {key: '', fingerprint: uid, browser: fingerprint_browser(), os: fingerprint_os(), useragent: fingerprint_useragent()};
    var realtimeQr = $interval(function () {
        $scope.dataQr.key = $scope.data[rand].key;
        rand = Math.floor(Math.random() * 9);
    }, 7000);
    $scope.isLogin = [];
    $scope.row = {fingerprint: uid};
    var realtimeUserSession = $interval(function () {
        LoginService.checkUserSession($scope.row)
            .then(function (response) {
                $scope.isLogin = response.data;
                console.log("status:" + response.status);
            })
            .catch(function (response) {
                console.error('Error checkUserSession:', response.status, response.data);
            })
            .finally(function () {
                if ($scope.isLogin != "") {
                    $scope.rowx = {qr: $scope.dataQr.key.toString() + $scope.isLogin.toString() + "-" + uid.toString()};
                    LoginService.loginToWeb($scope.rowx)
                        .then(function (response) {
                            $scope.stopRealtimeQr();
                            $scope.stopRealtimeUserSession();
                            $rootScope.globals.idUser = response.data.tusers.idUser;
                            $rootScope.globals.uname = response.data.tusers.uname;
                            $rootScope.globals.isVisibleLogin = true;
                            $location.path('/dashboard');
                            console.log("status:" + response.status);
                        })
                        .catch(function (response) {
                            console.error('Error loginToWeb:', response.status, response.data);
                        })
                        .finally(function () {
                            console.log("Task Finished.");
                        });
...
loginCtrl.$inject = ['$scope', 'LoginService', '$interval', '$rootScope', '$location'];
